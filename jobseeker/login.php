  ...
  <body class="bg-dark">
    ...
    <div class="container">
      <div class="card card-login mx-auto mt-5" id="formDiv">
        <div class="card-header">Login to JobSeeker Account</div>
        <div class="card-body">
          <form method="POST" action="login.php">
           ...
           <input type="submit" name="" class="btn btn-primary btn-block" value="Login">
            ...
            <div class="text-center">
                 <a class="d-block small mt-3" href="registerJobSeeker.php">I don't have JobSeeker Account</a>
...
session_start();
 if($_SERVER["REQUEST_METHOD"] == "POST")  {
    ...
    $jsEmail=$_POST['email'];
    $jsPassword=$_POST['password'];
    if(($jsEmail=="") && ($jsPassword="")){
      ...
      echo "Blank Field Aren't allowed. Please Input Email or Password";
            ...
            $query = "SELECT * FROM jobseeker  WHERE jsEmail='$jsEmail' AND jsPassword='$jsPassword'";
            $result = $crud->getData($query);
                     if($result){
                            foreach ($result as $key => $res) {
                              //while($res = mysqli_fetch_array($result)) {  
                             $_SESSION['jsId']= $res['jsId'] ; 
                             $_SESSION['jsName']= $res['jsName'] ;     
                             $_SESSION['jsEmail']= $res['jsEmail'] ;   

                              if (isset($_SESSION['jobId'])) {
                                $jobid=$_SESSION['jobId'];
                            header('location: ../applyJob.php?jobid='.$jobid);
                                exit();
                              }
                              else{
